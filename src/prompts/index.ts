    ...
    () =>
      userMessage(
        [
          ...
          "4. If attachments are needed, deepy_upload_file each local file and keep its fileId.",
          "5. deepy_estimate_generation — pass those ids in referenceFiles and get the token cost.",
          "6. Show the user the cost and get EXPLICIT approval.",
          "7. deepy_create_generation with confirmed=true and the SAME params used for the estimate",
          ...
          "9. deepy_get_result — the server fetches the media, saves it on the user's device,",
          "   and returns the local file path (small images/audio may also render inline;",
          "   videos are delivered as a local path, never via the Deepy app first).",
          "",
