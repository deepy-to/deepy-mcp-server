      ...
      if (!media.inline) {
        if (media.base64) {
          ...
          if (filePath) {
            ...
            return textResult(
              `${kind} for generation ${args.publicId} (#${index}) is ${media.contentType}, ~${sizeHuman}.\n` +
 ...
 */
async function saveLocalFile(bytes: Buffer, filename: string): Promise<string | undefined> {
