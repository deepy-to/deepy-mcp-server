 ...
 */
export function loadConfig(env: NodeJS.ProcessEnv = process.env): DeepyConfig {
  const apiKey = (env.DEEPY_API_KEY ?? "").trim();
  if (apiKey.length === 0) {
    throw new ConfigError(
      "DEEPY_API_KEY is not set. Provide your Deepy API key (sk_live_… or sk_test_…) via the " +
  ...
  if (rawBaseUrl.length === 0) {
    throw new ConfigError(
      "DEEPY_API_BASE_URL is not set. Provide the Deepy API base URL (dev stand: " +
  ...
  if (parsed.protocol !== "http:" && parsed.protocol !== "https:") {
    throw new ConfigError(
      `DEEPY_API_BASE_URL must be an http(s) URL; got "${parsed.protocol}" in "${rawBaseUrl}".`
    );
  }
