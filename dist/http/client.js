...
export class DeepyApiClient {
    ...
    maxDownloadResultBytes;
    constructor(config, options = {}) {
        this.baseUrl = config.baseUrl;
        this.apiKey = config.apiKey;
        const impl = options.fetchImpl ?? globalThis.fetch;
        if (typeof impl !== "function") {
            throw new Error("No fetch implementation available. Use Node >= 22 or pass options.fetchImpl.");
        }
        this.fetchImpl = impl;
        ...
        this.maxDownloadResultBytes = Math.max(this.maxInlineResultBytes, options.maxDownloadResultBytes ?? DEFAULT_MAX_DOWNLOAD_RESULT_BYTES);
    }
     ...
     */
    async getResultMedia(publicId, index) {
        const url = this.buildUrl(`/api/v1/public/generations/${encodeURIComponent(publicId)}/results/${index}`);
        const headers = {
            Authorization: `Bearer ${this.apiKey}`,
            Accept: "*/*",
        };
        ...
        try {
            ...
            if (cause instanceof DeepyApiError)
                throw cause;
    ...
    async execute(method, url, headers, body, timeoutMs) {
        const controller = new AbortController();
    ...
    toNetworkError(cause, aborted, timeoutMs) {
...
async function readCapped(response, maxBytes) {
    ...
    if (!body) {
        ...
        if (buffer.length > maxBytes)
            return { overCap: true, sizeBytes: buffer.length };
