...
export const uploadFileInputSchema = {
    filePath: z
        ...
        .min(1)
        .optional()
        .describe("Absolute local path to an image, video, or audio attachment. Preferred for files from the agent window."),
    base64: z
        .string()
        .min(1)
        .optional()
...
export function makeUploadFileHandler(ctx) {
    return (args) => runTool(ctx, async () => {
        const hasPath = args.filePath !== undefined;
        const hasBase64 = args.base64 !== undefined;
        if (hasPath === hasBase64) {
            return errorResult("[INVALID_INPUT] Provide exactly one source: filePath or base64 (with filename).");
        }
        const upload = hasPath
            ? await preparePathUpload(args.filePath, args.mimeType)
            : prepareBase64Upload(args.base64, args.filename, args.mimeType);
        if ("error" in upload)
            return errorResult(upload.error);
        const response = await ctx.client.uploadFile(upload);
        return textResult(formatJson(response) +
            `\n\nUpload complete. Use referenceFiles: ["${response.fileId}"] in both ` +
            "deepy_estimate_generation and deepy_create_generation.");
    });
}
export function registerUploadFile(server, ctx) {
    server.registerTool("deepy_upload_file", {
        title: "Upload a generation reference",
        description: "Upload an image, video, or audio attachment (up to 50 MiB) to Deepy and return its fileId. " +
...
async function preparePathUpload(filePath, requestedMimeType) {
    let handle;
    try {
...
function prepareBase64Upload(encoded, filename, requestedMimeType) {
    if (!filename) {
    ...
    if (encoded.length > MAX_BASE64_CHARS || !isStrictBase64(encoded)) {
        return {
            error: "[INVALID_INPUT] base64 is malformed or exceeds the 50 MiB decoded limit. " +
...
function resolveMimeType(requested, filename) {
    const value = (requested ?? MIME_BY_EXTENSION[extname(filename).toLowerCase()])?.toLowerCase();
    if (!value || !ALLOWED_MIME_TYPES.has(value)) {
        return {
            error: `[UNSUPPORTED_MEDIA_TYPE] Cannot determine a supported MIME type for "${filename}". ` +
...
function tooLargeMessage(sizeBytes) {
    return (`[FILE_TOO_LARGE] Attachment is ${sizeBytes} bytes; the MCP upload limit is ` +
