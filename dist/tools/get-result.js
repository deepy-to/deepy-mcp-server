...
export function makeGetResultHandler(ctx) {
    return (args) => runTool(ctx, async () => {
        ...
        const caption = `Result for generation ${args.publicId} (#${index}) — ${media.contentType}, ` +
...
export function registerGetResult(server, ctx) {
    server.registerTool("deepy_get_result", {
        ...
        description: "Fetch a COMPLETED generation's result media. The server fetches the bytes with its own API key and returns " +
