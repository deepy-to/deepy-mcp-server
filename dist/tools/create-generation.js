...
export const createGenerationInputSchema = {
    ...
    confirmed: z
        .boolean()
        .describe("MUST be true. Set to true ONLY after the user has seen the estimate and explicitly approved the charge."),
    idempotencyKey: z
        .string()
        .optional()
        .describe("Optional X-Idempotency-Key. If omitted, a stable key is derived from the request body so " +
...
const CONFIRM_REFUSAL = "Refusing to create a PAID generation: `confirmed` is not true. " +
...
export function makeCreateGenerationHandler(ctx) {
    return (args) => runTool(ctx, async () => {
        ...
        const idempotencyKey = normalizeIdempotencyKey(args.idempotencyKey) ?? idempotencyKeyForBody(body);
        const generation = await ctx.client.post("/api/v1/public/generations", {
            body,
            idempotencyKey,
        });
        return textResult(formatJson(generation) +
...
export function registerCreateGeneration(server, ctx) {
    server.registerTool("deepy_create_generation", {
        title: "Create a generation (paid)",
        description: "Start a PAID generation. Requires confirmed=true — the tool refuses otherwise and contacts no backend. " +
...
//# sourceMappingURL=create-generation.js.map